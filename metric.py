...
class GenericEvaluator:
    ...
    def bleu_metric(self, data, type='bleu'):
        ...
        gen, truth = [], []
        for row in data:
            gen.append(clean(row['generated_response']))
            truth.append(clean(row['response']))
    ...
    def tracker_metric(self, data, tracker_type):
        ...
        goal_accr /= total
        return precision, recall, f1, goal_accr
...
class KvretEvaluator(GenericEvaluator):
    ...
    def _extract_constraint(self, z):
        # get the intersection of all possible constraints and generated constrants
        z = z.split()
        if 'EOS_Z1' not in z:
            s = set(z)
        else:
